...
namespace TextRPG
{
    public class Player : Character
    {
        ...
        public void Move(int direction)
        {
            ...
            {
                if (RoomIndex.y == 0) return;
                Journal.Instance.Log("You moved North.");
                RoomIndex -= Vector2.up;
            }
            if (direction == 1)     //right(East)
            {
                if (RoomIndex.x == 0) return;
                Journal.Instance.Log("You moved East.");
                RoomIndex -= Vector2.right;
            }
            if (direction == 2)     //down(South)
            {
                if (RoomIndex.y == world.Dungeon.GetLength(0) - 1) return;
                Journal.Instance.Log("You moved South.");
                RoomIndex -= Vector2.down;
            }
            if (direction == 3)     //left(West)
            {
                if (RoomIndex.x == world.Dungeon.GetLength(0) - 1) return;
        ...
        public void Investigate()
        {
            ...
            else if (this.Room.Chest != null)
            {
                ...
                Journal.Instance.Log("You found a chest! What would you do?");
            }
